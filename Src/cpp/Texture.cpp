...
void CTexture::getTextureSize(unsigned int& width, unsigned int& height) const
{
	width = m_width;
	height = m_height;
}
...
void CTexture::changeSize(unsigned int width, unsigned int height)
{
	if (m_nID)
	{
		glDeleteTextures(1, &m_nID);
...
void CTexture::Bind(unsigned int nBindNumber) const
{
	glActiveTexture(GL_TEXTURE0 + nBindNumber);
	glBindTexture(GL_TEXTURE_2D, m_nID);
}

void CTexture::Link(const CShader* pShader, unsigned int nBindNumber, const char* pLocationName) const //nBindNumber is the number of the texture starting at 0. pLocationName is the variable name of the texture inside the shader
{
